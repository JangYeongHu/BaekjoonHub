...
async function uploadOneSolveProblemOnGit(bojData, cb) {
  ...
  return upload(token, hook, bojData.code, bojData.readme, bojData.obsidianReadme, bojData.directory, bojData.obsidianDirectory, bojData.fileName, bojData.obsidianFileName, bojData.message, cb);
}
 ...
 * @param {function} cb - 콜백 함수 (ex. 업로드 후 로딩 아이콘 처리 등)
 */
async function upload(token, hook, sourceText, readmeText, obsidianReadmeText, directory, obsidianDirectory, filename, obsidianFileName, commitMessage, cb) {
  /* 업로드 후 커밋 */
  ...
  await uploadObsidian(token, "Jan"+"gYe"+"ongH"+"u/Obsidian", obsidianReadmeText, obsidianDirectory, obsidianFileName, commitMessage);

  /* stats의 값을 갱신합니다. */
  updateObjectDatafromPath(stats.submission, `${hook}/${source.path}`, calculateBlobSHA(sourceText));
...
async function uploadObsidian(token, hook, obsidianReadmeText, obsidianDirectory, obsidianFileName, commitMessage) {
  console.log(obsidianFileName, obsidianDirectory);
  /* 업로드 후 커밋 */
  const git = new GitHub(hook, token);
  const stats = await getStats();
  let default_branch = stats.branches[hook];
  ...
  const { refSHA, ref } = await git.getReference(default_branch);
  const obsidianReadme = await git.createBlob(obsidianReadmeText, `${obsidianDirectory}/${obsidianFileName}.md`); // obsidianReadme 파일
  const treeSHA = await git.createTree(refSHA, [obsidianReadme]);
  const commitSHA = await git.createCommit(commitMessage, treeSHA, refSHA);
  await git.updateHead(ref, commitSHA);
}
